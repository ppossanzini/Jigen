...
namespace Jigen.Indexer
{
  ...
  public partial class SmallWorld<TItem, TDistance>
  {
    ...
    internal abstract class Node
    {
      ...
      protected Node(int id, TItem item, int maxLevel, Func<TItem, TItem, TDistance> distance, SmallWorldParameters smallWorldParameters)
      {
        this.Id = id;
        this.Item = item;
        ...
        {
          this.Connections.Add(new List<Node>(GetM(this.SmallWorldParameters.M, level)));
        }
      ...
      /// </summary>
      protected SmallWorldParameters SmallWorldParameters { get; private set; }
      ...
      public void AddConnection(Node newNeighbour, int level)
      {
        ...
        if (levelNeighbours.Count > GetM(this.SmallWorldParameters.M, level))
        {
    ...
    private class NodeAlg3 : Node
    {
      ...
      /// <param name="distance">The distance function for attached items.</param>
      /// <param name="smallWorldParameters">The parameters of the algorithm.</param>
      public NodeAlg3(int id, TItem item, int maxLevel, Func<TItem, TItem, TDistance> distance, SmallWorldParameters smallWorldParameters)
        : base(id, item, maxLevel, distance, smallWorldParameters)
      {
      }
      ...
      public override IList<Node> SelectBestForConnecting(IList<Node> candidates)
      {
        ...
        var result = new List<Node>(GetM(this.SmallWorldParameters.M, this.MaxLevel) + 1);
        while (candidatesHeap.Buffer.Any() && result.Count < GetM(this.SmallWorldParameters.M, this.MaxLevel))
        {
    ...
    private class NodeAlg4 : Node
    {
      ...
      /// <param name="distance">The distance function for attached items.</param>
      /// <param name="smallWorldParameters">The parameters of the algorithm.</param>
      public NodeAlg4(int id, TItem item, int maxLevel, Func<TItem, TItem, TDistance> distance, SmallWorldParameters smallWorldParameters)
        : base(id, item, maxLevel, distance, smallWorldParameters)
      {
      }
      ...
      public override IList<Node> SelectBestForConnecting(IList<Node> candidates)
      {
        ...
        var resultHeap = new BinaryHeap<Node>(new List<Node>(GetM(this.SmallWorldParameters.M, this.MaxLevel) + 1), closerIsLess);
        var candidatesHeap = new BinaryHeap<Node>(candidates, fartherIsLess);

        // expand candidates option is enabled
        if (this.SmallWorldParameters.ExpandBestSelection)
        {
        ...
        while (candidatesHeap.Buffer.Any() && resultHeap.Buffer.Count < GetM(this.SmallWorldParameters.M, this.MaxLevel))
        {
          ...
          else if (this.SmallWorldParameters.KeepPrunedConnections)
          {
        ...
        if (this.SmallWorldParameters.KeepPrunedConnections)
        {
          while (discardedHeap.Buffer.Any() && resultHeap.Buffer.Count < GetM(this.SmallWorldParameters.M, this.MaxLevel))
          {
