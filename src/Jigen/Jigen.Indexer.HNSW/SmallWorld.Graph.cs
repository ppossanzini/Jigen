    ...
    internal class Graph
    {
      ...
      public Graph(Func<TItem, TItem, TDistance> distance, SmallWorldParameters smallWorldParameters)
      {
        ...
        switch (this.SmallWorldParameters.NeighbourHeuristic)
        {
          case NeighbourSelectionHeuristic.SelectHeuristic:
            this.CreateNewNode = (id, item, level) => new NodeAlg4(id, item, level, distance, this.SmallWorldParameters);
            break;

          case NeighbourSelectionHeuristic.SelectSimple:
          default:
            this.CreateNewNode = (id, item, level) => new NodeAlg3(id, item, level, distance, this.SmallWorldParameters);
            break;
        }
      }
      ...
      public void Create(IList<TItem> items, Random generator)
      {
        ...
        for (id = 1; id < items.Count; ++id)
        {
          ...
          var newNode = this.CreateNewNode(id, items[id], RandomLevel(generator, this.SmallWorldParameters.LevelLambda));
          ...
          {
            var potentialNeighbours = KNearestAtLevel(bestPeer, newNode, this.SmallWorldParameters.ConstructionPruning, level);
      ...
      {
        var bestPeer = this._entryPoint;
        for (int level = this._entryPoint.MaxLevel; level > 0; --level)
        {
      ...
      internal string Print()
      {
        ...
        for (int level = this._entryPoint.MaxLevel; level >= 0; --level)
        {
          ...
          BFS(this._entryPoint, level, (node) =>
          {
