...
- **Phase 1 — API layer + auth.** Generate OpenAPI types, one service module per controller, Axios instance with base URL from `public/settings.json` (same pattern as Insight v1/v2). Route guard from the template.
  **Auth model (binding):** API calls authenticate with an OAuth2 **bearer token**, not the session cookie. The cookie from `POST /api/identity/login` is used *only* to complete the first `authorize` step. Flow: login (cookie) → `GET /api/connect/authorize` with `client_id=jigen-insight-spa`, `response_type=code`, PKCE S256 (required by the server), registered redirect URI `/auth/callback` → `POST /api/connect/token` (`grant_type=authorization_code` + `code_verifier`) → `access_token`/`refresh_token` → `Authorization: Bearer` on every request; renew via `grant_type=refresh_token` (`offline_access` scope). The seeded public client and redirect URIs live in `appsettings.json` under `JigenIdentity:DefaultClient` (dev redirect is port **5173** — keep the Vite dev server on that port or update the server config).
