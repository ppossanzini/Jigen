...
export function setupLoading() {
  // Fallback used only before the theme store has cached a value in localStorage;
  // kept in sync with the default `themeColor` in `src/theme/settings.ts` (the central theme config).
  const themeColor = localStg.get('themeColor') || '#B5E61D';
