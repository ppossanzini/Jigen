...
> {
   ...
   *
   * For example: You can show error message in this hook. It may also recover from the error by
   * returning a substitute `AxiosResponse` (e.g. after refreshing an expired auth token and
   * retrying the original request) — when it does, that response is used to resolve the original
   * call instead of rejecting it.
   *
   * @param error
   */
  onError: (error: AxiosError<ResponseData>) => void | AxiosResponse | Promise<void | AxiosResponse>;
}
...
export interface RequestInstanceCommon<State extends Record<string, unknown>> {
  ...
  state: State;
  /** the underlying axios instance, for callers that need to issue a raw retry (e.g. after `onError` recovers a token) */
  instance: AxiosInstance;
}
