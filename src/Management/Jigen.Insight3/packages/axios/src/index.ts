...
>(axiosConfig?: CreateAxiosDefaults, options?: Partial<RequestOption<ResponseData, ApiData, State>>) {
  ...
  instance.interceptors.response.use(
    ...
    async (error: AxiosError<ResponseData>) => {
      const recovered = await opts.onError(error);

      // `onError` recovered (e.g. refreshed an expired token and retried): resolve with that
      // response instead of rejecting, so the original caller never sees the failure.
      if (recovered) {
        return recovered;
      }
